...
import com.kosey.project.app.config.KafkaProperties;
...
class ProjectBaseAppKafkaResourceIT {
    ...
    void setup() {
        ...
        ProjectBaseAppKafkaResource kafkaResource = new ProjectBaseAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/project-base-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/project-base-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
